...
| ID | Status | Task | Priority | Complexity | Dependencies | Acceptance Criteria |
|---|---|---|---|---|---|---|
| 2.2.1 | In Progress | Build event-triggered entry point (single `employee_id`) | P0 | S | Phase 1 complete | Given a test `employee_id`, triggers OP-02 and OP-03 — built, subworkflow-linking glitch during setup (picker kept resolving both calls to OP-02) resolved by explicitly telling the builder OP-03 already exists/is published; confirmed in Workgraph diagram as two distinct steps with distinct subworkflow IDs |
| 2.2.2 | In Progress | Build parallel fan-out to OP-02 + OP-03 | P0 | M | 2.2.1 | Both Operators' calls are observably concurrent (not sequential) in Auto's execution trace — **not yet confirmed**: first test run reported both OP-02 and OP-03 as failed (`orch01_no_signal_available`) despite both being independently verified working (`1.2.7`/`1.3.8`) — an ORCH-01 input/output-mapping bug, not an OP-02/OP-03 defect. Fix in progress: checking employee_id input-field mapping and success/failure detection logic in the "Merge Risk Signals" step. Timing evidence so far also shows OP-02→OP-03 as sequential (17s gap), not overlapping — needs re-verification once the mapping bug is fixed |
