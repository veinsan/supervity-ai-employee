...
| R-05 | Airtable API rate limits hit during a full 60-worker cohort sweep or live demo | Low | Medium | Low | Retry/backoff config (`ARCHITECTURE.md` §7); cohort size is small enough that this is unlikely in practice | If hit during the live demo, the retry/backoff is itself demonstrable evidence of robustness (`OPERATORS.md` failure tables) rather than pure downside |
